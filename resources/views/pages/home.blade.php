    ...
    <div class="relative max-w-7xl mx-auto px-4 w-full">
        <div class="flex flex-col items-start max-w-2xl">
            ...
            <div class="flex flex-wrap gap-4" data-aos="fade-up" data-aos-delay="300">
                ...
                <a href="https://wa.me/{{ setting('whatsapp_link', '6285649150049') }}" class="group bg-white/10 backdrop-blur-md border border-white/30 text-white rounded-full px-8 py-3.5 font-medium hover:bg-white hover:text-dark-oak transition-all inline-flex items-center gap-2">
...
<section class="relative py-16 md:py-20 bg-linen overflow-hidden">
    ...
    <div class="relative max-w-7xl mx-auto px-4">
        ...
        <div class="flex flex-col gap-8 md:gap-12 max-w-4xl mx-auto">
            @foreach($tips as $tip)
            @php $bgUrl = $tip->background_image_url; @endphp
            <div class="relative p-6 md:p-8 rounded-2xl border-2 border-black/20 hover:border-black/50 transition-all w-full md:w-4/5 {{ $loop->even ? 'self-end md:self-end' : 'self-start' }} overflow-hidden group"
                 data-aos="{{ $loop->even ? 'fade-left' : 'fade-right' }}" data-aos-delay="{{ $loop->iteration * 80 }}">
                @if($bgUrl)
                <div class="absolute inset-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-105" style="background-image: url('{{ $bgUrl }}')"></div>
                ...
                <div class="relative z-10 flex items-center gap-4 mb-3">
                    <div class="w-12 h-12 rounded-full flex items-center justify-center {{ $bgUrl ? 'bg-white/20' : ($loop->even ? ' bg-terracotta/10' : ' bg-sage-green/10') }}">
                        <x-icons.tip :icon="$tip->icon ?? 'lightbulb'" class="w-5 h-5 {{ $bgUrl ? 'text-white' : 'text-sage-green' }}" />
                    </div>
                    <h3 class="font-playfair text-lg {{ $bgUrl ? 'text-white' : 'text-dark-oak' }}">{{ $tip->title }}</h3>
                </div>
                <p class="{{ $bgUrl ? 'text-white/90' : 'text-warm-gray' }} text-sm leading-relaxed relative z-10">{{ $tip->content }}</p>
            </div>
            @endforeach
        </div>
        @else
...
<section class="relative py-16 bg-gradient-to-r from-sage-green to-[#6b8f54] overflow-hidden">
    ...
    <div class="relative max-w-7xl mx-auto px-4">
        <div class="flex flex-col md:flex-row items-center justify-between gap-6" data-aos="fade-up">
            ...
            <a href="https://wa.me/{{ setting('whatsapp_link', '6285649150049') }}" class="group inline-flex items-center gap-2 bg-white text-sage-green rounded-full px-8 py-3.5 font-semibold hover:shadow-xl hover:shadow-black/10 hover:-translate-y-0.5 transition-all">
