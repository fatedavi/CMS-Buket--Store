...
@section('content')
<div x-data="categoryPreview()" class="mb-6">
    <div class="flex items-center justify-between mb-6">
        <h1 class="font-playfair text-2xl text-dark-oak">Tambah Kategori Baru</h1>
    ...
    <div class="grid grid-cols-1 gap-6" :class="showPreview ? 'lg:grid-cols-5' : ''">
        <div :class="showPreview ? 'lg:col-span-3' : ''">
            <div class="bg-white rounded-2xl border border-amber-100 p-6">
                <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data" id="form-category-create" class="space-y-5">
                    @csrf

                    <div>
                        <label class="block text-sm font-medium text-dark-oak mb-1">Nama Kategori</label>
                        <input type="text" name="name" value="{{ old('name') }}" x-model="form.name"
                    ...
                    <div class="flex gap-3 pt-2">
                        <button type="button" @click.prevent="openConfirm('save', 'Simpan Kategori', 'Simpan kategori baru?', '', 'form-category-create')" class="bg-sage-green text-white rounded-xl px-6 py-2.5 font-medium hover:brightness-110 transition-all">Simpan Kategori</button>
        ...
        <div x-show="showPreview" class="lg:col-span-2" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0">
            <div class="sticky top-8">
                <div class="bg-white rounded-2xl border border-amber-100 p-4 mb-4">
                    ...
                    {{-- Category card — matches homepage style exactly --}}
                    <div class="group relative h-40 rounded-xl overflow-hidden max-w-sm mx-auto">
                        <template x-if="form.imagePreview">
                            <img :src="form.imagePreview" alt="" class="w-full h-full object-contain p-2 group-hover:scale-110 transition-transform duration-700">
                        </template>
                        <template x-if="!form.imagePreview">
...
function clearImage(e) {
    e.stopPropagation();
    const alpine = document.querySelector('[x-data="categoryPreview()"]')?.__x;
    if (alpine) {
