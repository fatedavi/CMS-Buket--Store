...
@section('content')
<h1 class="font-playfair text-2xl text-dark-oak mb-6">Edit Kategori</h1>
    ...
    <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            ...
            <div id="drop-zone"
                ...
                <div id="image-preview" class="{{ $category->image ? '' : 'hidden' }}">
                    <img id="preview-img" src="{{ $category->image_url }}" alt="Preview" class="mx-auto max-h-48 rounded-xl object-contain shadow">
                    <p id="preview-name" class="text-xs text-warm-gray mt-2 truncate">{{ $category->image ? 'Gambar saat ini' : '' }}</p>
        ...
        <div>
            <label class="flex items-center gap-2">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }} class="rounded border-sand text-sage-green focus:ring-sage-green">
...
dropZone.addEventListener('drop', (e) => {
    ...
    if (file && file.type.startsWith('image/')) {
        ...
        showPreview(file);
        document.querySelector('input[name="remove_image"]') && (document.querySelector('input[name="remove_image"]').checked = false);
    }
});
