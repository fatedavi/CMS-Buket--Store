...
@section('content')
<div x-data="articlePreview()" class="mb-6">
    <div class="flex items-center justify-between mb-6">
        <h1 class="font-playfair text-2xl text-dark-oak">Tambah Artikel Baru</h1>
    ...
    <div class="grid grid-cols-1 gap-6" :class="showPreview ? 'lg:grid-cols-5' : ''">
        <div :class="showPreview ? 'lg:col-span-3' : ''">
            <div class="bg-white rounded-2xl border border-amber-100 p-6">
                <form action="{{ route('admin.articles.store') }}" method="POST" enctype="multipart/form-data" id="form-article-create" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-sm font-medium text-dark-oak mb-1">Judul Artikel</label>
                        <input type="text" name="title" value="{{ old('title') }}" x-model="form.title"
                    ...
                    <div>
                        ...
                        <textarea name="excerpt" rows="3" x-model="form.excerpt"
                                  class="w-full border border-sand rounded-xl px-4 py-2.5 focus:border-sage-green focus:outline-none focus:ring-2 focus:ring-sage-green/20">{{ old('excerpt') }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-dark-oak mb-1">Konten (HTML)</label>
                        <textarea name="content" rows="12"
                                  class="w-full border border-sand rounded-xl px-4 py-2.5 focus:border-sage-green focus:outline-none focus:ring-2 focus:ring-sage-green/20 font-mono text-sm">{{ old('content') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <div id="drop-zone"
                            ...
                            <div id="upload-placeholder" :class="form.imagePreview ? 'hidden' : ''">
                            ...
                            <div id="image-preview" :class="form.imagePreview ? '' : 'hidden'">
                                <img id="preview-img" :src="form.imagePreview" alt="Preview" class="mx-auto max-h-48 rounded-xl object-contain shadow">
                                <p id="preview-name" class="text-xs text-warm-gray mt-2 truncate" x-text="form.imageName"></p>
                    ...
                    <div>
                        ...
                        <input type="text" name="date" value="{{ old('date', now()->format('j F Y')) }}"
                               class="w-full border border-sand rounded-xl px-4 py-2.5 focus:border-sage-green focus:outline-none focus:ring-2 focus:ring-sage-green/20">
                        <p class="text-xs text-warm-gray mt-1">Format: 1 January 2024. Kosongkan untuk menggunakan tanggal hari ini.</p>
                    </div>

                    <div class="flex gap-3 pt-4">
                        <button type="button" @click.prevent="openConfirm('save', 'Simpan Artikel', 'Simpan artikel baru?', '', 'form-article-create')" class="bg-sage-green text-white rounded-xl px-6 py-2.5 font-medium hover:brightness-110 transition-all">Simpan Artikel</button>
                    ...
                    <div class="bg-white border border-amber-100 rounded-xl overflow-hidden group max-w-sm mx-auto">
                        <div class="aspect-[4/3] overflow-hidden bg-cream">
                            <template x-if="form.imagePreview">
                                <img :src="form.imagePreview" alt="" class="w-full h-full object-contain p-2 group-hover:scale-105 transition-transform">
                            </template>
                            <template x-if="!form.imagePreview">
...
function clearImage(e) {
    e.stopPropagation();
    const alpine = document.querySelector('[x-data="articlePreview()"]')?.__x;
    if (alpine) {
