...
<div class="bg-white rounded-2xl border border-amber-100 p-6">
    <form action="{{ route('admin.tips.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
...
function showBgPreview(file) {
    ...
    reader.onload = (e) => {
        document.querySelector('#drop-zone #preview-img').src = e.target.result;
        document.querySelector('#drop-zone #preview-name').textContent = file.name;
        document.querySelector('#drop-zone #upload-placeholder').classList.add('hidden');
        document.querySelector('#drop-zone #image-preview').classList.remove('hidden');
    };
...
function clearBgImage(e) {
    ...
    document.querySelector('#drop-zone #upload-placeholder').classList.remove('hidden');
    document.querySelector('#drop-zone #image-preview').classList.add('hidden');
}
</script>
@endpush
@endsection