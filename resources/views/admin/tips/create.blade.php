...
@section('content')
<div x-data="tipsPreview()" class="mb-6">
    <div class="flex items-center justify-between mb-6">
        <h1 class="font-playfair text-2xl text-dark-oak">Tambah Tips Baru</h1>
    ...
    <div class="grid grid-cols-1 gap-6" :class="showPreview ? 'lg:grid-cols-5' : ''">
        <div :class="showPreview ? 'lg:col-span-3' : ''">
            <div class="bg-white rounded-2xl border border-amber-100 p-6">
                <form action="{{ route('admin.tips.store') }}" method="POST" enctype="multipart/form-data" id="form-tip-create" class="space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea name="content" rows="6" x-model="form.content"
                                  class="w-full border border-sand rounded-xl px-4 py-2.5 focus:border-sage-green focus:outline-none focus:ring-2 focus:ring-sage-green/20">{{ old('content') }}</textarea>
                    ...
                    <div class="flex gap-3 pt-4">
                        <button type="button" @click.prevent="openConfirm('save', 'Simpan Tips', 'Simpan tips baru?', '', 'form-tip-create')" class="bg-sage-green text-white rounded-xl px-6 py-2.5 font-medium hover:brightness-110 transition-all">Simpan Tips</button>
        ...
        <div x-show="showPreview" class="lg:col-span-2" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0">
            <div class="sticky top-8">
                <div class="bg-white rounded-2xl border border-amber-100 p-4 mb-4">
                    ...
                    <div class="relative p-6 rounded-2xl border-2 overflow-hidden group max-w-sm mx-auto"
                         :class="form.bgPreview ? 'border-black/20 hover:border-black/50' : 'border-black/20 hover:border-black/50'">
                        <template x-if="form.bgPreview">
                            <div>
                                <div class="absolute inset-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-105" :style="'background-image: url(' + form.bgPreview + ')'"></div>
                        ...
                        <div class="relative z-10 flex items-center gap-4 mb-3">
                            <div class="w-12 h-12 rounded-full flex items-center justify-center"
                                 :class="form.bgPreview ? 'bg-white/20' : 'bg-sage-green/10'">
                                <div :class="form.bgPreview ? 'text-white' : 'text-sage-green'">
                                    <x-icons.tip :icon="old('icon', 'lightbulb')" class="w-5 h-5" />
                                </div>
                            </div>
                            <h3 class="font-playfair text-lg" :class="form.bgPreview ? 'text-white' : 'text-dark-oak'"
                                x-text="form.title || 'Judul Tips'"></h3>
                        </div>
                        <p class="text-sm leading-relaxed relative z-10"
                           :class="form.bgPreview ? 'text-white/90' : 'text-warm-gray'"
...
function clearBg(e) {
    e.stopPropagation();
    const alpine = document.querySelector('[x-data="tipsPreview()"]')?.__x;
    if (alpine) {
